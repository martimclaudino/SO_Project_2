...
int main(int argc, char *argv[])
{
  ...
  int pipe_fd;
  if ((pipe_fd = open(server_pipe_path, O_RDONLY | O_NONBLOCK)) < 0)
  {
...
void *respond_client()
{
  while (1)
  {
    ...
    // Opening the pipes
    if ((resp_fd = open(resp_pipe_path, O_WRONLY | O_NONBLOCK)) < 0)
    {
      perror("Failed to open response pipe");
      return NULL;
    }
    if ((req_fd = open(req_pipe_path, O_RDONLY | O_NONBLOCK)) < 0)
    {
    ...
    if ((notif_fd = open(notif_pipe_path, O_WRONLY | O_NONBLOCK)) < 0)
    {
      write_all(resp_fd, opcode + "1", 2);
