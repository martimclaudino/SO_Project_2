...
void sort_keys_and_values(char keys[][MAX_STRING_SIZE], char values[][MAX_STRING_SIZE], size_t num_pairs)
{
...
void kvs_show(int fd)
{
  ...
  for (int i = 0; i < TABLE_SIZE; i++)
  {
    ...
    while (keyNode != NULL)
    {
      ...
      write(fd, buffer, strlen(buffer)); // Write the formatted string to the file descriptor
...
int kvs_backup(int fd, int out_fd, const char *job_path, size_t pathLength, int *bck_counter, DIR *dir)
{
  ...
  if (pid == 0)
  {
    ...
    snprintf(bck_path, pathLength + (size_t)(2 + (*bck_counter / 10)), "%s-%d.bck", job_path, *bck_counter);
    ...
    for (int i = 0; i < TABLE_SIZE; i++)
    {
      ...
      while (keyNode != NULL)
      {
        ...
        char buffer[len + 1]; // Allocate buffer with the required length
        snprintf(buffer, sizeof(buffer), "(%s, %s)\n", keyNode->key, keyNode->value);
        write(bck_fd, buffer, strlen(buffer)); // Write the formatted string to the file descriptor
        keyNode = keyNode->next;               // Move to the next node
      }
    }
    close(bck_fd);
