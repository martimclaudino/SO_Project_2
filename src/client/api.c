...
int req_fd_api, resp_fd_api, notif_fd_api;
...
int kvs_connect(char *req_pipe_path, char *resp_pipe_path,
  ...
  // Opening the pipes
  if ((resp_fd_api = open(resp_pipe_path, O_RDONLY)) < 0) {
    perror("Failed to open response pipe");
    return 1;
  }

  if ((notif_fd_api = open(notif_pipe_path, O_RDONLY)) < 0) {
    perror("Failed to open notification pipe");
    return 1;
  }

  if ((req_fd_api = open(req_pipe_path, O_WRONLY)) < 0) {
    perror("Failed to open request pipe");
    return 1;
  }

  *notif_fd_main = notif_fd_api;
  *req_fd_main = req_fd_api;
  *resp_fd_main = resp_fd_api;
  printf("opened pipes\n");
  char server_connection_result[3];

  read_all(resp_fd_api, server_connection_result, 3, NULL);
  ...
  if (server_connection_result[1] == '1') {  // Failed to connect
    close(req_fd_api);
    close(resp_fd_api);
    close(notif_fd_api);
    return 1;
...
int kvs_disconnect() {
  ...
  if (write_all(req_fd_api, "2", 1) == -1) {
    perror("Failed to write to response pipe");
    return 1;
  }
  char response[3];
  if (read_all(resp_fd_api, response, 3, NULL) == -1) {
...
int kvs_subscribe(const char *key) {
  ...
  if (write_all(req_fd_api, subscrition, 41) == -1) {
    perror("Failed to write to response pipe");
    return 1;
  }
  // Wait for response
  char server_subscription_result[3];
  if (read_all(resp_fd_api, server_subscription_result, 3, NULL) == -1) {
    perror("Failed to read from response pipe");
    return 1;
  }
...
int kvs_unsubscribe(const char *key) {
  ...
  if (write_all(req_fd_api, unsubscrition, 41) == -1) {
    perror("Failed to write to response pipe");
    return 1;
  }
  char server_unsubscription_result[3];
  if (read_all(resp_fd_api, server_unsubscription_result, 3, NULL) == -1) {
    perror("Failed to read from response pipe");
    return 1;
  }
  for (int i = 0; i < 3; i++) {
